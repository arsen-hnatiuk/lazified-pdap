...
class LazifiedPDAP:
    ...
    def lsi(
        ...
        while point_steps < self.stop_search:
            condition_2_rhs = np.maximum(
                (np.maximum(P_vals - max(self.alpha, max_P_A), 0) + Psi / self.M)
                / (4 * self.R),
    ...
    def lsi_simple(
        ...
        while point_steps < self.stop_search:
            gradients = grad_P(lsi_set)
            hessians = gradients if self.projection == "sphere" else hess_P(lsi_set)
            for i, (point, gradient, hessian) in enumerate(
                zip(lsi_set, gradients, hessians)
            ):
                try:
                    d = np.linalg.solve(hessian, -gradient)  # Newton step
                    if self.projection == "sphere":
                    ...
                    lsi_set[i] = point + 0.1 * gradient
            lsi_set = self.project_into_domain(lsi_set).copy()
            point_steps += 1

        P_vals = p_norm(lsi_set)
        hat_ind = np.argmax(P_vals - original_P_vals)
        x_hat = lsi_set[hat_ind]
        check_ind = np.argmax(P_vals)
        x_check = lsi_set[check_ind]
    ...
    def lpdap(self, tol: float, u_0: Measure = Measure(), Psi_0: float = 1) -> tuple:
        ...
        while Phi_k > tol:
            ...
            x_hat_lsi, x_tilde_lsi, x_check_lsi, lsi_set = self.lsi(
                p_u, u, epsilon, Phi_A
            )
            if len(x_tilde_lsi):
                x_k = x_tilde_lsi
            ...
            if len(x_check_lsi):
                Phi_k_lsi = (
                    np.max(self.M * (np.abs(p_u(x_check_lsi))[0] - self.alpha), 0) + q_u
                )
            else:
                Phi_k_lsi = 0
            ...
            # LSI step
            if len(x_hat_lsi):
                v_tilde = self.local_measure_constructor(p_u, u, x_hat_lsi, lsi_set)
                nu_constant = 4 * (
                    32
                    * self.L
                    * self.M**2
    ...
    def lpdap_simple(self, tol: float, u_0: Measure, Psi_0: float = 1) -> tuple:
        ...
        while Phi_k > tol:
            ...
            x_hat_lsi, x_check_lsi, lsi_set = self.lsi_simple(p_u, u)

            # Build Phi_k
            Phi_k = np.max(self.M * (np.abs(p_u(x_check_lsi))[0] - self.alpha), 0) + q_u
            ...
            if Phi_k > q_u:
                v_k = Measure([x_check_lsi], [self.M * np.sign(p_u(x_check_lsi)[0])])
            else:
                v_k = Measure()
            u_plus_hat = u * (1 - eta) + v_k * eta
